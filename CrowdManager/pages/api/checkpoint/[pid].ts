...
import { Checkpoint, updateCheckpoint, deleteCheckpoint, ICheckpoint } from "../../../model/Checkpoint"
...
const handler = async (req, res) => {
    ...
    const put = async(checkpoint: ICheckpoint) => {
      if(checkpoint) {
        try {
          ...
          res.status(200).json({ response: 'success', checkpoint: new_checkpoint })
        }
        catch (ex) {
          logger.error(ex)
          res.status(500).json({ response: 'error', message: "General application error", checkpoint: "" })
        }
      } else {
        logger.error('Checkpoint body not provided')
        res.status(500).json({ response: 'error', message: "Body not provided", checkpoint: "" })
      }
    }
    ...
    const delete_method = async(id: string) => {
      if(id) {
        ...
        res.status(500).json({ response: 'error', message: "ID not provided", checkpoint: "" })
      }
    }
