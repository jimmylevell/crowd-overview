...
def export_engine(model, im, file, half, dynamic, simplify, workspace=4, verbose=False):
    ...
    try:
        assert (
            im.device.type != "cpu"
        ), "reid_export running on CPU but must be on GPU, i.e. `python export.py --device 0`"
        try:
...
def export_tflite(file, half, prefix=colorstr("TFLite:")):
    ...
    try:
        check_requirements(
            ("openvino2tensorflow", "tensorflow", "tensorflow_datasets")
        )  # requires openvino-dev: https://pypi.org/project/openvino-dev/
        import openvino.inference_engine as ie
...
if __name__ == "__main__":
    ...
    if torchscript:
        export_torchscript(
            extractor.model.eval(), im, args.weights, optimize=True
        )  # opset 12
    if onnx:
        f = export_onnx(
