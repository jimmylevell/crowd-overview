...
def visualize_object_tracking(img, detections):
    for detection in detections:
        cv2.putText(
            img,
            detection.get_short_description(),
...
def filter_detections(detections):
    return [
        detection
        for detection in detections
        if detection.score > 0.5 and detection.detections > 5
    ]
...
def run(files=[], name="", object_class=0):
    ...
    for img in files:
        ...
        mot_output(
            detections=detections, class_id=object_class, file=file, frame_idx=frame_idx
        )
